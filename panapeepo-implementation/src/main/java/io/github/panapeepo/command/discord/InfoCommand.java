...
public class InfoCommand {
    ...
    public void handle(@Nonnull DiscordCommandSender sender) {
        ...
        this.panapeepo.sendDefaultEmbed(sender.getChannel(), user, embed -> {
            ...
            embed.addField("Shards", String.format("%d / %d (current #%d)", this.panapeepo.getShardManager().getShardsRunning(), this.panapeepo.getShardManager().getShardsTotal(), user.getJDA().getShardInfo().getShardId() + 1), true);
